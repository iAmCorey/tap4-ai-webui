...
function NavDrawerItem({ isActive, name }: { isActive: boolean; name: string }) {
  return (
    <li
      className={cn(
        'flex h-[28px] w-full items-center justify-between rounded-[4px] border border-transparent bg-white pl-[14px] pr-2',
        isActive && 'border-white bg-white',
      )}
    >
      <div className={cn('size-3 rounded-full bg-gray-800', isActive && 'bg-orange-400')} />
      <div className={cn('text-sm text-black', isActive && 'font-bold text-black')}>{name}</div>
    </li>
  );
}
...
export default function NavigationDrawer({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed right-0 top-16 z-[99999] h-[calc(100%-64px)] w-[276px] transform bg-slate-200 shadow-lg transition-transform duration-300',
